...
static void remoteinstall_network_update(ui_view* view, void* data, float* progress, char* text) {
    ...
    if(sock >= 0) {
        ...
        if(remoteinstall_network_recvwait(networkData->clientSocket, &size, sizeof(size), 0) != sizeof(size)) {
            error_display_errno(NULL, NULL, errno, "Echec de la lecture du payload.");
        ...
        if(size >= DOWNLOAD_URL_MAX * INSTALL_URLS_MAX) {
            error_display(NULL, NULL, "Payload trop grand.");
        ...
        if(urls == NULL) {
            error_display(NULL, NULL, "Echec de l'allocation du tampon d'URL.");
        ...
        if(remoteinstall_network_recvwait(networkData->clientSocket, urls, size, 0) != size) {
            error_display_errno(NULL, NULL, errno, "Echec de lecture de l'URL.");
        ...
        action_install_url("Installer depuis l'URL recue ?", urls, NULL, data, NULL, remoteinstall_network_close_client, NULL);
        ...
        error_display_errno(NULL, NULL, errno, "Echec de l'ouverture du socket.");
    ...
    snprintf(text, PROGRESS_TEXT_MAX, "Attente de connexion...\nIP: %s\nPort: 5000", inet_ntoa(addr));
}
...
static void remoteinstall_receive_urls_network() {
    ...
    if(sock < 0) {
        error_display_errno(NULL, NULL, errno, "Echec de l'ouverture du socket serveur.");
    ...
    if(bind(data->serverSocket, (struct sockaddr*) &server, sizeof(server)) < 0) {
        error_display_errno(NULL, NULL, errno, "Echec de la liaison du socket serveur.");
    ...
    if(listen(data->serverSocket, 5) < 0) {
        error_display_errno(NULL, NULL, errno, "Echec de l'ecoute sur le socket serveur.");

        remoteinstall_network_free_data(data);
        return;
    }

    info_display("Recevoir les URL(s)", "B: Retour", false, data, remoteinstall_network_update, NULL);
}
...
static void remoteinstall_qr_update(ui_view* view, void* data, float* progress, char* text) {
    ...
    if(!installData->capturing) {
        ...
        if(R_FAILED(capRes)) {
            ...
            error_display_res(NULL, NULL, capRes, "Echec du demarrage de la capture video.");
    ...
    if(installData->captureInfo.finished) {
        ...
        if(R_FAILED(installData->captureInfo.result)) {
            error_display_res(NULL, NULL, installData->captureInfo.result, "Erreur lors de la capture des\n images de la caméra.");
        }
    ...
    for(int i = 0; i < qrCount; i++) {
        ...
        if(err == 0) {
            ...
            action_install_url("Installer depuis le code QR ?", (const char*) qrData.payload, NULL, NULL, NULL, NULL, NULL);
            return;
        }
    }

    snprintf(text, PROGRESS_TEXT_MAX, "Attente d'un code QR...");
}
...
static void remoteinstall_scan_qr_code() {
    ...
    if(data->qrContext == NULL) {
        error_display(NULL, NULL, "Echec de la création du contexte QR.");
    ...
    if(quirc_resize(data->qrContext, QR_IMAGE_WIDTH, QR_IMAGE_HEIGHT) != 0) {
        error_display(NULL, NULL, "Échec du redimensionnement du\ncontexte QR.");
    ...
    if(data->captureInfo.buffer == NULL) {
        error_display(NULL, NULL, "Echec de la creation du tampon d'image.");
    ...
    info_display("Installer par code QR", "B: Retour, X: Inverser la camera", false, data, remoteinstall_qr_update, remoteinstall_qr_draw_top);
}
...
static void remoteinstall_manually_enter_urls_onresponse(ui_view* view, void* data, SwkbdButton button, const char* response) {
    if(button == SWKBD_BUTTON_CONFIRM) {
        ...
        action_install_url("Installer a partir des URL saisies ?", response, NULL, NULL, NULL, NULL, NULL);
    }
}

static void remoteinstall_manually_enter_urls() {
    kbd_display("Entrer l'URL(s)", "", SWKBD_TYPE_NORMAL, SWKBD_MULTILINE, SWKBD_NOTEMPTY_NOTBLANK, DOWNLOAD_URL_MAX * INSTALL_URLS_MAX, NULL, remoteinstall_manually_enter_urls_onresponse);
}
...
static void remoteinstall_repeat_last_request() {
    ...
    if(textBuf != NULL) {
        if(remoteinstall_get_last_urls(textBuf, DOWNLOAD_URL_MAX * INSTALL_URLS_MAX)) {
            action_install_url("Installer a partir de la ou des\ndernières URL demandees ?", textBuf, NULL, NULL, NULL, NULL, NULL);
        } else {
            prompt_display_notify("Echec", "Aucune URL recente.", COLOR_TEXT, NULL, NULL, NULL);
        }
...
static void remoteinstall_forget_last_request() {
    ...
    if(R_SUCCEEDED(forgetRes)) {
        prompt_display_notify("Succes", "Derniere URL oubliee.", COLOR_TEXT, NULL, NULL, NULL);
    } else {
        error_display_res(NULL, NULL, forgetRes, "Impossible d'oublier la ou les\ndernieres URL demandees.");
    }
}

static list_item receive_urls_network = {"Recevoir les URLs via le reseau", COLOR_TEXT, remoteinstall_receive_urls_network};
static list_item scan_qr_code = {"Scanner le code QR", COLOR_TEXT, remoteinstall_scan_qr_code};
static list_item manually_enter_urls = {"Entrer manuellement l'URL", COLOR_TEXT, remoteinstall_manually_enter_urls};
static list_item repeat_last_request = {"Repeter la derniere requete", COLOR_TEXT, remoteinstall_repeat_last_request};
static list_item forget_last_request = {"Oublier la derniere requete", COLOR_TEXT, remoteinstall_forget_last_request};
...
void remoteinstall_open() {
    list_display("Installation distante", "A: Selection, B: Retour", NULL, remoteinstall_update, NULL);
}
