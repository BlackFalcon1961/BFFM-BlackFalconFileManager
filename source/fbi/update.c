...
static void update_check_update(ui_view* view, void* data, float* progress, char* text) {
                    ...
                    for(u32 i = 0; i < json_array_size(assets); i++) {
                        ...
                        if(json_is_object(val)) {
                            ...
                            if(json_is_string(assetName) && json_is_string(assetUrl)) {
                                if(strncmp(json_string_value(assetName), fs_get_3dsx_path() != NULL ? "BFFM.3dsx" : "BFFM.cia", json_string_length(assetName)) == 0) {
    ...
    if(hasUpdate) {
        action_install_url("Mettre a jour vers la derniere version ?", updateURL, fs_get_3dsx_path(), NULL, NULL, NULL, NULL);
    } else {
        if(R_FAILED(res)) {
            error_display_res(NULL, NULL, res, "Erreur de la recherche.");
        } else {
            prompt_display_notify("Termine", "Aucune mise a jour disponible.", COLOR_TEXT, NULL, NULL, NULL);
        }
    }
}

void update_open() {
    info_display("Verification des mises a jour...", "", false, NULL, update_check_update, NULL);
}
