...
static void action_new_folder_onresponse(ui_view* view, void* data, SwkbdButton button, const char* response) {
    ...
    if(button == SWKBD_BUTTON_CONFIRM) {
        ...
        if(R_SUCCEEDED(res)) {
            ...
            prompt_display_notify("Succes", "Dossier cree.", COLOR_TEXT, NULL, NULL, NULL);
        } else {
            error_display_res(NULL, NULL, res, "Echec de la creation du dossier.");
        }
    }
...
void action_new_folder(linked_list* items, list_item* selected) {
    ...
    if(data == NULL) {
        error_display(NULL, NULL, "Echec de l'allocation de nouvelles donnees de dossier.");
