...
static void action_delete_title_update(ui_view* view, void* data, float* progress, char* text) {
    ...
    if(R_FAILED(res)) {
        error_display_res(info, task_draw_title_info, res, "Echec de la suppression.");
    } else {
        linked_list_remove(deleteData->items, deleteData->selected);
        task_free_title(deleteData->selected);

        prompt_display_notify("Succes", "Application supprimee.", COLOR_TEXT, NULL, NULL, NULL);
    }
...
static void action_delete_title_onresponse(ui_view* view, void* data, u32 response) {
    if(response == PROMPT_YES) {
        info_display("Suppression de l'app", "", false, data, action_delete_title_update, action_delete_title_draw_top);
    } else {
        free(data);
    }
}
...
void action_delete_title(linked_list* items, list_item* selected) {
    action_delete_title_internal(items, selected, "Supprimer l'app selectionnee ?", false);
}

void action_delete_title_ticket(linked_list* items, list_item* selected) {
    action_delete_title_internal(items, selected, "Supprimer l'app selectionnee\net le ticket ?", true);
}