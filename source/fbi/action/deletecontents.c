...
static bool action_delete_error(void* data, u32 index, Result res, ui_view** errorView) {
    *errorView = error_display_res(data, action_delete_draw_top, res, "Echec de la suppression du contenu.");
    return true;
}
...
static void action_delete_update(ui_view* view, void* data, float* progress, char* text) {
    ...
    if(deleteData->deleteInfo.finished) {
        ...
        if(R_SUCCEEDED(deleteData->deleteInfo.result)) {
            prompt_display_notify("Succes", "Supprime.", COLOR_TEXT, NULL, NULL, NULL);
        }
...
static void action_delete_onresponse(ui_view* view, void* data, u32 response) {
    ...
    if(response == PROMPT_YES) {
        ...
        if(R_SUCCEEDED(res)) {
            info_display("Suppression", "Appuyez sur B pour annuler.", true, data, action_delete_update, action_delete_draw_top);
        } else {
            error_display_res(NULL, NULL, res, "Echec de l'initialisation de la suppression.");
...
static void action_delete_loading_update(ui_view* view, void* data, float* progress, char* text)  {
    ...
    if(loadingData->popData.finished) {
        ...
        if(R_SUCCEEDED(loadingData->popData.result)) {
            ...
            error_display_res(NULL, NULL, loadingData->popData.result, "Echec du remplissage de la liste de contenu.");
    ...
    snprintf(text, PROGRESS_TEXT_MAX, "Recuperation de la liste de contenu ...");
}
...
static void action_delete_internal(linked_list* items, list_item* selected, const char* message, bool recursive, bool includeBase, bool ciasOnly, bool ticketsOnly) {
    ...
    if(data == NULL) {
        error_display(NULL, NULL, "Echec de l'attribution des donnees de suppression.");
    ...
    if(R_FAILED(listRes)) {
        error_display_res(NULL, NULL, listRes, "Echec de l'initialisation du remplissage\nde la liste de contenu.");
...
void action_delete_file(linked_list* items, list_item* selected) {
    action_delete_internal(items, selected, "Supprimer le fichier selectionne ?", false, true, false, false);
}

void action_delete_dir(linked_list* items, list_item* selected) {
    action_delete_internal(items, selected, "Supprimer le repertoire actuel ?", true, true, false, false);
}

void action_delete_dir_contents(linked_list* items, list_item* selected) {
    action_delete_internal(items, selected, "Supprimer tout le contenu du repertoire actuel ?", true, false, false, false);
}

void action_delete_dir_cias(linked_list* items, list_item* selected) {
    action_delete_internal(items, selected, "Supprimer tous les CIA du repertoire actuel ?", false, false, true, false);
}

void action_delete_dir_tickets(linked_list* items, list_item* selected) {
    action_delete_internal(items, selected, "Supprimer tous les tickets du repertoire actuel ?", false, false, false, true);
}