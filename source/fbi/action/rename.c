...
static void action_rename_onresponse(ui_view* view, void* data, SwkbdButton button, const char* response) {
    ...
    if(button == SWKBD_BUTTON_CONFIRM) {
        ...
        if(R_SUCCEEDED(res)) {
            if(strncmp(selected->name, "<repertoire actuel>", LIST_ITEM_NAME_MAX) != 0 && strncmp(selected->name, "<fichier actuel>", LIST_ITEM_NAME_MAX) != 0) {
            ...
            prompt_display_notify("Succes", "Renomme.", COLOR_TEXT, NULL, NULL, NULL);
        } else {
            error_display_res(NULL, NULL, res, "Echec du renommage.");
        }
    }
...
void action_rename(linked_list* items, list_item* selected) {
    ...
    if(data == NULL) {
        error_display(NULL, NULL, "Échec de l'attribution des\ndonnées de changement de nom.");
