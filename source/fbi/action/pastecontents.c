...
static void action_paste_contents_update(ui_view* view, void* data, float* progress, char* text) {
    ...
    if(pasteData->pasteInfo.finished) {
        ...
        if(R_SUCCEEDED(pasteData->pasteInfo.result)) {
            prompt_display_notify("Succes", "Contenu colle.", COLOR_TEXT, NULL, NULL, NULL);
        }
    ...
    *progress = pasteData->pasteInfo.currTotal != 0 ? (float) ((double) pasteData->pasteInfo.currProcessed / (double) pasteData->pasteInfo.currTotal) : 0;
    snprintf(text, PROGRESS_TEXT_MAX, "%lu / %lu\n%.2f %s / %.2f %s\n%.2f %s/s, ETAT %s", pasteData->pasteInfo.processed, pasteData->pasteInfo.total,
...
static void action_paste_contents_onresponse(ui_view* view, void* data, u32 response) {
    ...
    if(response == PROMPT_YES) {
        ...
        if(R_SUCCEEDED(res)) {
            info_display("Collage du contenu", "Appuyez sur B pour annuler.", true, data, action_paste_contents_update, action_paste_contents_draw_top);
        } else {
            error_display_res(NULL, NULL, res, "Echec de l'initialisation de\nl'operation de collage.");
...
static void action_paste_contents_loading_update(ui_view* view, void* data, float* progress, char* text)  {
    ...
    if(loadingData->popData.finished) {
        ...
        if(R_SUCCEEDED(loadingData->popData.result)) {
            ...
            prompt_display_yes_no("Confirmation", "Coller le contenu du presse-papiers\ndans le repertoire actuel ?", COLOR_TEXT, loadingData->pasteData, action_paste_contents_draw_top, action_paste_contents_onresponse);
        } else {
            error_display_res(NULL, NULL, loadingData->popData.result, "Echec du remplissage de la liste\nde contenu du presse-papiers.");
    ...
    snprintf(text, PROGRESS_TEXT_MAX, "Recuperation de la liste de\ncontenu du presse-papiers...");
}

void action_paste_contents(linked_list* items, list_item* selected) {
    if(!clipboard_has_contents()) {
        prompt_display_notify("Erreur", "Presse-papiers vide.", COLOR_TEXT, NULL, NULL, NULL);
        return;
    }

    paste_contents_data* data = (paste_contents_data*) calloc(1, sizeof(paste_contents_data));
    if(data == NULL) {
        error_display(NULL, NULL, "Echec de l'allocation des\ndonnees de contenu colle.");
    ...
    if(R_FAILED(targetCreateRes)) {
        error_display_res(NULL, NULL, targetCreateRes, "Echec de la creation de l'element\nde fichier cible.");
    ...
    if(R_FAILED(listRes)) {
        error_display_res(NULL, NULL, listRes, "Echec de l'initialisation du remplissage de\nla liste de contenu du presse-papiers.");
