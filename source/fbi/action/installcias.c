...
static Result action_install_cias_open_dst(void* data, u32 index, void* initialReadBlock, u64 size, u32* handle) {
    ...
    if(R_SUCCEEDED(APT_CheckNew3DS(&n3ds)) && !n3ds && ((info->ciaInfo.titleId >> 28) & 0xF) == 2) {
        ui_view* view = prompt_display_yes_no("Confirmation", "Le logiciel est destine aux nouveaux systemes 3DS.\nContinuer ?", COLOR_TEXT, data, action_install_cias_draw_top, action_install_cias_n3ds_onresponse);
        if(view != NULL) {
...
bool action_install_cias_error(void* data, u32 index, Result res, ui_view** errorView) {
    *errorView = error_display_res(data, action_install_cias_draw_top, res, "Echec de l'installation");
    return true;
}
...
static void action_install_cias_update(ui_view* view, void* data, float* progress, char* text) {
    ...
    if(installData->installInfo.finished) {
        ...
        if(R_SUCCEEDED(installData->installInfo.result)) {
            prompt_display_notify("Succes", "Installation terminee.", COLOR_TEXT, NULL, NULL, NULL);
        }
    ...
    *progress = installData->installInfo.currTotal != 0 ? (float) ((double) installData->installInfo.currProcessed / (double) installData->installInfo.currTotal) : 0;
    snprintf(text, PROGRESS_TEXT_MAX, "%lu / %lu\n%.2f %s / %.2f %s\n%.2f %s/s, ETAT %s", installData->installInfo.processed, installData->installInfo.total,
...
static void action_install_cias_onresponse(ui_view* view, void* data, u32 response) {
    ...
    if(response == PROMPT_YES) {
        ...
        if(R_SUCCEEDED(res)) {
            info_display("Installation du/des CIA(s)", "Appuyez sur B pour annuler.", true, data, action_install_cias_update, action_install_cias_draw_top);
        } else {
            error_display_res(NULL, NULL, res, "Echec du lancement de l'installation du CIA.");
...
static void action_install_cias_loading_update(ui_view* view, void* data, float* progress, char* text)  {
    ...
    if(loadingData->popData.finished) {
        ...
        if(R_SUCCEEDED(loadingData->popData.result)) {
            ...
            error_display_res(NULL, NULL, loadingData->popData.result, "Echec du remplissage de la liste des CIA.");
    ...
    snprintf(text, PROGRESS_TEXT_MAX, "Recuperation de la liste des CIA...");
}
...
static void action_install_cias_internal(linked_list* items, list_item* selected, bool (*filter)(void* data, const char* name, u32 attributes), void* filterData, const char* message, bool delete) {
    ...
    if(R_FAILED(targetCreateRes)) {
        error_display_res(NULL, NULL, targetCreateRes, "Echec de la creation de l'element de fichier cible.");
    ...
    if(loadingData == NULL) {
        error_display(NULL, NULL, "Echec de l'allocation des donnees de chargement.");
    ...
    if(R_FAILED(listRes)) {
        error_display_res(NULL, NULL, listRes, "Echec de l'initialisation de la liste des CIA.");
...
void action_install_cia(linked_list* items, list_item* selected) {
    action_install_cias_internal(items, selected, NULL, NULL, "Installer les CIA selectionnes ?", false);
}

void action_install_cia_delete(linked_list* items, list_item* selected) {
    action_install_cias_internal(items, selected, NULL, NULL, "Installer et supprimer les CIA selectionnes ?", true);
}
...
void action_install_cias(linked_list* items, list_item* selected, bool (*filter)(void* data, const char* name, u32 attributes), void* filterData) {
    action_install_cias_internal(items, selected, filter, filterData, "Installer tous les CIA dans le repertoire actuel ?", false);
}
...
void action_install_cias_delete(linked_list* items, list_item* selected, bool (*filter)(void* data, const char* name, u32 attributes), void* filterData) {
    action_install_cias_internal(items, selected, filter, filterData, "Installer et supprimer tous les CIA\n du repertoire actuel ?", true);
}