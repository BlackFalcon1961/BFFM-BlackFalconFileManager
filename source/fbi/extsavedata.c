...
static list_item browse_user_save_data = {"Chercher donnees utilisateur", COLOR_TEXT, action_browse_user_ext_save_data};
static list_item browse_spotpass_save_data = {"Chercher donnees SpotPass", COLOR_TEXT, action_browse_boss_ext_save_data};
static list_item delete_save_data = {"Supprimer les donnees", COLOR_TEXT, action_delete_ext_save_data};
...
static void extsavedata_action_open(linked_list* items, list_item* selected) {
    ...
    list_display("Action sauvegardes externes", "A: Selection, B: Retour", data, extsavedata_action_update, extsavedata_action_draw_top);
}
...
static void extsavedata_options_update(ui_view* view, void* data, linked_list* items, list_item* selected, bool selectedTouched) {
    ...
    if(linked_list_size(items) == 0) {
        extsavedata_options_add_entry(items, "Afficher carte SD", &listData->showSD);
        extsavedata_options_add_entry(items, "Afficher la NAND", &listData->showNAND);
...
static void extsavedata_options_open(extsavedata_data* data) {
    list_display("Options", "A: Basculer, B: Retour", data, extsavedata_options_update, NULL);
}
...
void extsavedata_open() {
    ...
    list_display("Sauvegardes externes", "A: Selection, B: Retour, X: Rafraichir, Select: Options", data, extsavedata_update, extsavedata_draw_top);
}