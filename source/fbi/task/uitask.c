...
void task_draw_file_info(ui_view* view, void* data, float x1, float y1, float x2, float y2) {
    ...
    if(info->attributes & (FS_ATTRIBUTE_DIRECTORY | FS_ATTRIBUTE_HIDDEN | FS_ATTRIBUTE_ARCHIVE | FS_ATTRIBUTE_READ_ONLY)) {
        ...
        if(info->attributes & FS_ATTRIBUTE_DIRECTORY) {
            infoTextPos += snprintf(infoText + infoTextPos, sizeof(infoText) - infoTextPos, "Repertoire");
            needsSeparator = true;
        }
        ...
        if(info->attributes & FS_ATTRIBUTE_HIDDEN) {
            ...
            infoTextPos += snprintf(infoText + infoTextPos, sizeof(infoText) - infoTextPos, "Masque");
            needsSeparator = true;
        }
        ...
        if(info->attributes & FS_ATTRIBUTE_READ_ONLY) {
            ...
            infoTextPos += snprintf(infoText + infoTextPos, sizeof(infoText) - infoTextPos, "Lecture seule");
            needsSeparator = true;
        }
    } else {
        infoTextPos += snprintf(infoText + infoTextPos, sizeof(infoText) - infoTextPos, "Aucune");
    }

    infoTextPos += snprintf(infoText + infoTextPos, sizeof(infoText) - infoTextPos, "\n");

    if(!(info->attributes & FS_ATTRIBUTE_DIRECTORY)) {
        infoTextPos += snprintf(infoText + infoTextPos, sizeof(infoText) - infoTextPos, "Taille : %.2f %s\n",
        ...
        if(info->isCia && info->ciaInfo.loaded) {
            ...
            infoTextPos += snprintf(infoText + infoTextPos, sizeof(infoText) - infoTextPos,
                                    "ID de l'application : %016llX\n"
                                            "Version : %hu (%d.%d.%d)\n"
                                            "Region : %s\n"
                                            "Taille installee : %.2f %s",
            ...
            infoTextPos += snprintf(infoText + infoTextPos, sizeof(infoText) - infoTextPos, "ID du ticket : %016llX", info->ticketInfo.titleId);
        }
    }
...
void task_draw_pending_title_info(ui_view* view, void* data, float x1, float y1, float x2, float y2) {
    ...
    snprintf(infoText, sizeof(infoText),
             "ID de l'application en attente : %016llX\n"
                     "Type : %s\n"
                     "Version : %hu (%d.%d.%d)",
             info->titleId,
             info->mediaType == MEDIATYPE_NAND ? "NAND" : info->mediaType == MEDIATYPE_SD ? "Carte SD" : "Carte de jeu",
...
void task_draw_system_save_data_info(ui_view* view, void* data, float x1, float y1, float x2, float y2) {
    ...
    snprintf(infoText, sizeof(infoText), "ID de la sauvegarde systeme : %08lX", info->systemSaveDataId);
...
void task_draw_ticket_info(ui_view* view, void* data, float x1, float y1, float x2, float y2) {
    ...
    if(info->loaded) {
        ...
        snprintf(infoText, sizeof(infoText), "ID de l'application : %016llX", info->titleId);
