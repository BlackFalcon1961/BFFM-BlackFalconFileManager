...
static list_item delete_file = {"Supprimer", COLOR_TEXT, action_delete_file};

static list_item install_cia = {"Installer CIA", COLOR_TEXT, action_install_cia};
static list_item install_and_delete_cia = {"Installer et supprimer le CIA", COLOR_TEXT, action_install_cia_delete};

static list_item install_ticket = {"Installer le ticket", COLOR_TEXT, action_install_ticket};
static list_item install_and_delete_ticket = {"Installer et supprimer le ticket", COLOR_TEXT, action_install_ticket_delete};

static list_item delete_dir = {"Supprimer", COLOR_TEXT, action_delete_dir};
static list_item copy_all_contents = {"Copier tout", COLOR_TEXT, NULL};
static list_item delete_all_contents = {"Supprimer tout", COLOR_TEXT, action_delete_dir_contents};
...
static list_item install_and_delete_all_cias = {"Installer et supprimer tous les CIAs", COLOR_TEXT, action_install_cias_delete};
static list_item delete_all_cias = {"Supprimer tous les CIAs", COLOR_TEXT, action_delete_dir_cias};

static list_item install_all_tickets = {"Installer tous les tickets", COLOR_TEXT, action_install_tickets};
static list_item install_and_delete_all_tickets = {"Installer et supprimer tous les tickets", COLOR_TEXT, action_install_tickets_delete};
static list_item delete_all_tickets = {"Supprimer tous les tickets", COLOR_TEXT, action_delete_dir_tickets};
    ...
    if(selected != NULL && (selected->data != NULL || selected == &copy || selected == &copy_all_contents) && (selectedTouched || (hidKeysDown() & KEY_A))) {
        ...
        if(selected == &copy || selected == &copy_all_contents) {
            ...
            if(R_SUCCEEDED(res = clipboard_set_contents(actionData->parent->archive, info->path, selected == &copy_all_contents))) {
                prompt_display_notify("Fini", selected == &copy_all_contents ? "Copie du contenu effectuee" : (info->attributes & FS_ATTRIBUTE_DIRECTORY) ? "Copie du dossier actuel effectuee" : "Copie du fichier effectuee", COLOR_TEXT, info, task_draw_file_info, NULL);
            } else {
                error_display_res(info, task_draw_file_info, res, "Erreur de la copie.");
            }
...
static void files_action_open(linked_list* items, list_item* selected, files_data* parent) {
    ...
    if(data == NULL) {
        error_display(NULL, NULL, "Echec de l'allocation des donnees\n d'action des fichiers.");
    ...
    list_display((((file_info*) selected->data)->attributes & FS_ATTRIBUTE_DIRECTORY) ? "Action du dossier" : "Action du fichier", "A: Selection, B: Retour", data, files_action_update, files_action_draw_top);
}
...
static void files_options_update(ui_view* view, void* data, linked_list* items, list_item* selected, bool selectedTouched) {
    ...
    if(linked_list_size(items) == 0) {
        files_options_add_entry(items, "Afficher les elements caches", &listData->showHidden);
        files_options_add_entry(items, "Afficher les dossiers", &listData->showDirectories);
        files_options_add_entry(items, "Afficher les fichiers", &listData->showFiles);
        files_options_add_entry(items, "Afficher les CIAs", &listData->showCias);
        files_options_add_entry(items, "Afficher les tickets", &listData->showTickets);
    }
}

static void files_options_open(files_data* data) {
    list_display("Options", "A: Basculer, B: Retour", data, files_options_update, NULL);
}
...
static void files_repopulate(files_data* listData, linked_list* items) {
    ...
    if(R_FAILED(res)) {
        error_display_res(NULL, NULL, res, "Echec du lancement du remplissage\n de la liste de fichiers.");
    }
...
static void files_update(ui_view* view, void* data, linked_list* items, list_item* selected, bool selectedTouched) {
    ...
    if(selected != NULL && selected->data != NULL && (selectedTouched || (hidKeysDown() & KEY_A))) {
        ...
        if((fileInfo->attributes & FS_ATTRIBUTE_DIRECTORY) && strncmp(selected->name, "<repertoire actuel>", LIST_ITEM_NAME_MAX) != 0) {
    ...
    if(listData->populateData.finished && R_FAILED(listData->populateData.result)) {
        error_display_res(NULL, NULL, listData->populateData.result, "Echec du remplissage de la liste des fichiers.");
...
void files_open(FS_ArchiveID archiveId, FS_Path archivePath) {
    ...
    if(data == NULL) {
        error_display(NULL, NULL, "Echec de l'allocation des donnees des fichiers.");
    ...
    if(R_FAILED(res = fs_open_archive(&data->archive, archiveId, archivePath))) {
        error_display_res(NULL, NULL, res, "Echec de l'ouverture de l'archive\n de liste de fichiers.");

        files_free_data(data);
        return;
    }

    list_display("Fichiers", "A: Selection, B: Betour, X: Rafraichir, Select: Options", data, files_update, files_draw_top);
}
...
void files_open_nand_warning(FS_ArchiveID archive) {
    prompt_display_yes_no("Confirmation", "La modification de la NAND est dangereuse\n et peut rendre le systeme inutilisable.\nSoyez donc certain de ce que vous faites.\n\nContinuer ?", COLOR_TEXT, (void*) archive, NULL, files_open_nand_warning_onresponse);
}
