...
void init() {
    ...
    if(R_FAILED(romfsRes)) {
        error_panic("Impossible de monter RomFS: %08lX", romfsRes);
        return;
    }

    if(R_FAILED(init_services())) {
        if(!attempt_patch_pid()) {
            error_panic("La backdoor du noyau n'est pas installée.\nVeuillez lancer un exploit du noyau\n et réessayer.");
            return;
        }

        Result initRes = init_services();
        if(R_FAILED(initRes)) {
            error_panic("Erreur de l'initialisation du service: %08lX", initRes);
            return;
        }
    }
    ...
    if(R_FAILED(cpuRes)) {
        error_panic("Échec de la définition de la limite de\n temps du CPU: %08lX", cpuRes);
        return;
    }
