...
static list_item launch_title = {"Lancer l'application", COLOR_TEXT, action_launch_title};
static list_item delete_title = {"Supprimer l'application", COLOR_TEXT, action_delete_title};
static list_item delete_title_ticket = {"Supprimer l'app et le ticket", COLOR_TEXT, action_delete_title_ticket};
static list_item extract_smdh = {"Extraire le fichier SMDH", COLOR_TEXT, action_extract_smdh};
static list_item import_seed = {"Importer la graine", COLOR_TEXT, action_import_seed};
static list_item browse_save_data = {"Explorer les sauvegardes", COLOR_TEXT, action_browse_title_save_data};
static list_item import_save_data = {"Importer la sauvegarde", COLOR_TEXT, action_import_twl_save};
static list_item export_save_data = {"Exporter la sauvegarde", COLOR_TEXT, action_export_twl_save};
static list_item erase_save_data = {"Supprimer les sauvegardes", COLOR_TEXT, action_erase_twl_save};
static list_item import_secure_value = {"Importer les donnees securisees", COLOR_TEXT, action_import_secure_value};
static list_item export_secure_value = {"Exporter les donnees securisees", COLOR_TEXT, action_export_secure_value};
static list_item delete_secure_value = {"Supprimer les donnees securisees", COLOR_TEXT, action_delete_secure_value};
...
static void titles_action_open(linked_list* items, list_item* selected) {
    ...
    list_display("Action des apps", "A: Selection, B: Retour", data, titles_action_update, titles_action_draw_top);
}
...
static void titles_options_update(ui_view* view, void* data, linked_list* items, list_item* selected, bool selectedTouched) {
    ...
    if(linked_list_size(items) == 0) {
        titles_options_add_entry(items, "Afficher la carte de jeu", &listData->showGameCard);
        titles_options_add_entry(items, "Afficher la carte SD", &listData->showSD);
        titles_options_add_entry(items, "Afficher la NAND", &listData->showNAND);
...
static void titles_options_open(titles_data* data) {
    list_display("Options", "A: Basculer, B: Retour", data, titles_options_update, NULL);
}
...
void titles_open() {
    ...
    list_display("Applications", "A: Selection, B: Retour, X: Rafraichir, Select: Options", data, titles_update, titles_draw_top);
}